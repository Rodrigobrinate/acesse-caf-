...
const [msg, setMsg] = useState("")
  ...
  useEffect(() => {
    api
      .get("/historic2")
      .then((response) => setCoffee2(response.data)) //response.data.map((item)=> console.log(item.colaborador)))
      .catch((err) => {
        console.error("ops! ocorreu um erro" + err);
      });
  }, []);
  ...
  return (
    ...
    <div className="list-coffee">
      <h1>Colaboradores no café hoje</h1>
      <ul>
      { coffee2.map( (item) => <li>{ item.colaborador }<span>{(((item.minutos )/1000)/60).toFixed(0) /*((((new Date().getTime() - new Date(item.data).getTime())/1000)/60)/60).toFixed(0)+":"+Math.floor((((new Date().getTime() - new Date(item.data).getTime())/1000)/60).toFixed(0)%60)+":"+ Math.floor((((new Date().getTime() - new Date(item.data).getTime())/1000)%60)) */}</span></li>) }
      </ul>
    </div>
